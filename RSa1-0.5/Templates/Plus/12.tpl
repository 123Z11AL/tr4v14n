...
if($golds['16'] == 0) {
mysql_query("UPDATE ".TB_PREFIX."users set b4 = '".mktime(date("H"),date("i"), date("s"),date("m") , date("d")+7, date("Y"))."' where `username`='".$session->username."'") or die(mysql_error());
} else {
mysql_query("UPDATE ".TB_PREFIX."users set b4 = '".($golds['16']+604800)."' where `username`='".$session->username."'") or die(mysql_error());
}
