...
class Village {
	...
	private function LoadMovements() {
		...
		$movement = array();
		$movement3[from] = $database->getMovement(3,$this->wid,0);
		$movement3[to] = $database->getMovement(3,$this->wid,1);		
		$movement4[from] = $database->getMovement(4,$this->wid,0);
		$movement4[to] = $database->getMovement(4,$this->wid,1);		 
		$movement5[from] = $database->getMovement(5,$this->wid,0);
		$movement5[to] = $database->getMovement(5,$this->wid,1);		
		
		// Unimos todos los arrays
		$movement[from] = array_merge($movement3[from],$movement4[from], $movement5[from]);
		$movement[to] = array_merge($movement3[to],$movement4[to], $movement5[to]);							
		// Clasificamos los arrays
		$movement[from] = $multisort->sorte($movement[from], "'endtime'", true, 2);
		$movement[to] = $multisort->sorte($movement[to], "'endtime'", true, 2);		
		return $movement;
	}
};
$village = new Village;
