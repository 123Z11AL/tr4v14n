...
class Building {	
	...
	private function upgradeBuilding($id) {
		...
		if($this->allocated < $this->maxConcurrent) {
			...
			if($database->addBuilding($village->wid,$id,$village->resarray['f'.$id.'t'],$loop,$time,1)) {
	...
	private function constructBuilding($id,$tid) {
		...
		if($this->allocated < $this->maxConcurrent) {
			...
			if($this->meetRequirement($tid)) {
				if($database->addBuilding($village->wid,$id,$tid,$loop,$time,1)) {
	...
	private function finishAll() {
		...
		foreach($this->buildArray as $jobs) {
			...
			if($database->query($q)) {
				...
				$database->addCP($$jobs['wid'],$resource['cp'],0);
